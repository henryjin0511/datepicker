...
!function (window) {
	...
	Datepicker.prototype.getTriValue = function(){
		var _this = this;
		var opts = _this.opts;
		var triEle = opts.tarEle;
		var triTagName = triEle.tagName.toLowerCase();
		if(triTagName === 'input'){
			return triEle.value;
		}else {
	...
	Datepicker.prototype.setTriValue = function (value) {
		var _this = this;
		var opts = _this.opts;
		var triEle = opts.tarEle;
		var triTagName = triEle.tagName.toLowerCase();
		if(triTagName === 'input'){
			...
			triEle.innerText = value;
		}
	};

	Datepicker.prototype.initData = function (first) {
		var _this = this;
		var triValue = _this.getTriValue();
		if(!first){
		...
		if(dateArr.length >= 3){
			dateArr.length = 6;
			dateArr[3] = dateArr[3] === undefined?'00':dateArr[3];
			dateArr[4] = dateArr[4] === undefined?'00':dateArr[4];
			dateArr[5] = dateArr[5] === undefined?'00':dateArr[5];
			if(dateArr[0] < 100 || dateArr[1] < 0 || dateArr[1] > 12 || dateArr[2] < 1 || dateArr[2] > 31 || dateArr[3] < 0 || dateArr[3] > 23 || dateArr[4] < 0 || dateArr[4] > 59 || dateArr[5] < 0 || dateArr[3] > 59){
				_this.setTriValue('');
	...
	Datepicker.prototype.createDom = function () {
		...
		_this.oDiv.id = 'datepicker-box'+order;
		...
		domStr += '<div id="datepicker-default" class="datepicker-default">'+
			...
			'			<span class="datepicker-year-show-year"></span>'+
			'	    </div>'+
			'       <div>'+
			'		    <table class="datepicker-year-middle">'+
			'			    <tbody>'+
			'                   <tr><td><span data-view="year" data-year="prev" href="javascript:;">&laquo;</span></td><td><span></span></td><td><span></span></td></tr>'+
		'                       <tr><td><span></span></td><td><span></span></td><td><span></span></td></tr>'+
			'                   <tr><td><span></span></td><td><span></span></td><td><span></span></td></tr>'+
			'                   <tr><td><span></span></td><td><span></span></td><td><span data-view="year" data-year="next" href="javascript:;">&raquo;</span></td></tr>'+
			'			    </tbody>'+
			'		    </table>'+
			'		</div>'+
			'	</div>'+
			'<div class="datepicker-month">'+
			'		<div class="datepicker-month-top">'+
			...
			(function () {
				...
				for(;i<24;i++){
					if((i+1)%4 === 0 && (i+1)!==24){
						str += '<td><span data-view="hour" data-hour="'+i+'">'+Datepicker.fill(i)+'</span></td></tr><tr>';
					}else{
						str += '<td><span data-view="hour" data-hour="'+i+'">'+Datepicker.fill(i)+'</span></td>';
					}
				}
				return str;
			})()+
		...
		_this.oDiv = document.getElementById('datepicker-box'+order);
	...
	Datepicker.prototype.render = function () {
		...
		var monthDays = [31,28+(function () {
			var tYear = showDateObj.getFullYear();
	...
	Datepicker.prototype.renderYearSelect = function (tYear) {
		...
		var oYearDivMiddle = Datepicker.getByClass(oYearDiv,'datepicker-year-middle')[0];
		if(tYear === 'prev'){
		...
		endYear = startYear+9;
		var i = 0 , spans = oYearDivMiddle.getElementsByTagName('span');
		for(;i<spans.length;i++){
			spans[i].className = '';
			if(i !== 0 && i !== spans.length-1){
				Datepicker.data(spans[i],'view','year');
				Datepicker.data(spans[i],'year',(startYear+i-1));
				spans[i].innerText = startYear+i-1;
		...
		Datepicker.getByClass(oDiv,'datepicker-year-show-year')[0].innerHTML = startYear + '-' + endYear;
	...
	Datepicker.prototype.renderMonthSelect = function (tMonth) {
		...
		var oMonthDivMiddle = Datepicker.getByClass(oMonthDiv,'datepicker-month-middle')[0];
		var i = 0 , spans = oMonthDivMiddle.getElementsByTagName('span');
		for(;i<spans.length;i++){
			spans[i].className = '';
			if(i+1 === tMonth){
				spans[i].className = 'cur';
	...
	Datepicker.prototype.getPosition = function () {
		...
		var rect = triEle.getBoundingClientRect();
		var rectTop = rect.top - document.documentElement.clientTop;
		var rectBottom = rect.bottom - document.documentElement.clientTop;
		var rectLeft = rect.left - document.documentElement.clientLeft;
		var wHeight = document.documentElement.clientHeight;
		var wScrollTop = document.documentElement.scrollTop;
		var oHeight = _this.oDiv.offsetHeight;
		var top = 0;
		if((rectTop + oHeight > wHeight) && (rectTop > oHeight)){
			top = wScrollTop + rectTop - oHeight;
		}else{
			top = wScrollTop + rectBottom;
		}
		_this.oDiv.style.top = top + 'px';
		_this.oDiv.style.left = rectLeft+ 'px';
	};
	...
	Datepicker.prototype.returnDate = function(){
		...
		if(typeof opts.change === 'function'){
			opts.change.call(_this,returnValue);
		}else{
			_this.setTriValue(returnValue);
		}
		_this.close();
